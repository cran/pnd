...
test_that("gradients are correct", {
  ...
  g <- Grad(f, x, h = 0.01)
  expect_identical(attr(g, "step.size.method"), "user-supplied")
  expect_named(g, c("A", "B", "C"))
})

test_that("Gradient step is auto-selected well", {
  f <- function(x) sum(sin(x))
  expect_identical(attr(Grad(x = 1:3, f), "step.size.method"), "default")
  expect_identical(attr(Grad(x = 1:3, f, h = 0.01), "step.size.method"), "user-supplied")
  expect_identical(attr(Grad(x = 1:3, f, h = "CR"), "step.size.method"), "CR")
  expect_identical(attr(Grad(x = 1:3, f, h = "SW"), "step.size.method"), "SW")
...
test_that("parallelisation of Grad works", {
  expect_identical(Grad(x = 1:3, FUN = sum, cores = 1),
...
test_that("Grad can accept dot arguments", {
  ...
  expect_identical(Grad(f, x = 0, a0 = 1, h = 1e-5), Grad(sin, x = 1, h = 1e-5))
  expect_identical(attr(Grad(f, x = 0, a0 = 1, h = "SW"), "step.search")$exitcode, 0L)
})
